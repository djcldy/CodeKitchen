mousedown: function (state) {
    ...
    let fixedPts = getStairNode(rooms, points);

    //select node
    let selected = this.selectNodes(graph, fixedPts);
    if (selected) {
      editor.recordLastState();
  ...
  function getStairNode(rooms, points){
    const stairPts = [];
  ...
  function rotateStair(graph, rooms) {
    const roomId = this.selectRoom(graph, rooms);
    const stairRooms = rooms.filter((r) => r.isStair && r.id === roomId);
    if (stairRooms.length > 0) { 
        editor.recordLastState();
        const room = stairRooms[0];
        let midPts = this.getMidPoints(room, points);
